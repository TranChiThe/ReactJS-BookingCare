...
class SearchBar extends Component {
    ...
    async componentDidMount() {
        ...
        let searchData = this.state.searchTerm ? this.state.searchTerm : ''
        let specialty = this.state.selectedSpecialty ? this.state.selectedSpecialty.value : '';
        let clinic = this.state.selectedClinic ? this.state.selectedClinic.value : '';
        this.doctorSearch(searchData, specialty, clinic)
    }
    ...
    doctorSearch = async (searchData, specialtyId, clinicId) => {
        let res = await doctorSearch(searchData, specialtyId, clinicId);
        if (res && res.errCode === 0) {
            this.setState({
                listDoctorFromParent: res.data ? res.data : []
            })
        }
    }
    ...
    handleKeyPress = (event) => {
        ...
        if (event.key === 'Enter') {
            this.handleDoctorSearch(searchTerm, selectedSpecialty, selectedClinic);
        }
    };
    ...
    handleSuggestionClick = async (suggestion) => {
        ...
        this.setState({ searchTerm: this.props.language === LANGUAGES.VI ? nameVi : nameEn, suggestions: [] });
    ...
    handleDoctorSearch = async (searchTerm, specialty, clinic) => {
        let res = await doctorSearch(searchTerm, specialty, clinic);
        if (res && res.errCode === 0) {
            this.setState({
    ...
    render() {
        ...
        return (
            <React.Fragment>
                ...
                <div className="search-container">
                    <div className='selected-options'>
                        <div className=" select-specialty">
                            <Select
                                ...
                                placeholder={<FormattedMessage id="patient.doctor-search.specialty" />}
                                isClearable
                        ...
                        <div className=" select-clinic">
                            <Select
                                options={listClinic}
                                onChange={this.handleOnchangeSelectDoctorInfo}
                                placeholder={<FormattedMessage id="patient.doctor-search.clinic" />}
                                isClearable
                                name="selectedClinic"
                            />
                        </div>
                    ...
                    <div className=" search-item">
                        <input
                            ...
                            placeholder={this.props.intl.formatMessage({ id: "patient.doctor-search.doctor" })}
                    ...
                    {suggestions.length > 0 && (
                        <ul className="suggestions">
                            {suggestions.map((doctor, index) => {
                                ...
                                let nameEn = `${doctor.firstName} ${doctor.lastName}`;
                                return (
                                    <li key={index} onClick={() => this.handleSuggestionClick(doctor)}>
                                        {/* <img src={doctor.image} alt={doctor.firstName} /> */}
                                        <div className="doctor-info">
                                            <span className="doctor-name">
                                                {this.highlightMatchingText(this.props.language === LANGUAGES.VI ? nameVi : nameEn, searchTerm)}
                                            </span>
                                        </div>
                                    </li>
                                );
                ...
                <AllDoctors
                    arrDoctorFromParent={this.state.listDoctorFromParent}
                />
                <HomeFooter />
            </React.Fragment>
        );
