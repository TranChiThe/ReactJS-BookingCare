...
class SearchBar extends Component {
  ...
  async componentDidMount() {
    ...
    let clinic = this.state.selectedClinic
      ...
      : "";
    let page = 1;
    this.doctorSearch(searchData, specialty, clinic, page);
  }
  ...
  doctorSearch = async (searchTerm, specialtyId, clinicId, page = 1) => {
    const res = await doctorSearch(searchTerm, specialtyId, clinicId, page, 2);
    if (res && res.errCode === 0) {
      this.setState({
        listDoctorFromParent: res.data,
  ...
  handlePageChange = (pageNumber) => {
    const { searchTerm, selectedSpecialty, selectedClinic } = this.state;
    this.doctorSearch(
      searchTerm,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    getAllRequiredDoctorInfo: () =>
      dispatch(actions.fetchAllRequiredDoctorStart()),
    getDetailInfoDoctor: (data) =>
      dispatch(actions.fetchDetailInforDoctorStart(data)),
  };
};
